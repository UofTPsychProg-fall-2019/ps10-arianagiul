...
Q1 <- ipip.l %>%
  ...
  labs(title = "BMI per amount of exercise by gender", x = "Amount of exercise")
Q1
...
Q2a <- ggplot(ipip,aes(x=logMedInc,y=BMI, color=gender))+
  ...
  labs(x = "Income", title = "Relationship between income and BMI by gender")
Q2a
...
Q3 <- ipip %>% 
  ...
  labs(y="income", title = "Relationship between income and exercise by gender")
...
Q4 <- ipip.l %>%
  ...
  labs(title = 'Gender differences in personality by BMI', x = "BMI category", y = "Strength of trait")
Q4
...
Q5 <- ipip.g %>%
  ...
  labs(title = "Mean persoality trait by BMI and gender", y = "Trait mean", x = "BMI category")
Q5
